...
void producer(){
    ...
    ptr.store(p, std::memory_order_release); // dependency-ordered-before
}

void consumer(){
    std::string* p2;
    while (!(p2 = ptr.load(std::memory_order_relaxed)));//carries-a-dependency-to
    std::cout << "*p2: " << *p2 << std::endl;//there
